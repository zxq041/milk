...
app.get('/api/seed-database', async (req, res) => {
    try {
        ...
        for (const collection of collections) { await db.collection(collection.name).drop(); }
    ...
    } catch (err) { res.status(500).json({ message: "Wystąpił błąd.", error: err.message }); }
});
...
app.get('/api/data', async (req, res) => {
    try {
        ...
        res.json({ users, products, categories, orders, reservations, logs, holidays: holidays.map(h => h.date), workSessions, activeSessions: activeSessionsDoc?.sessions || [] });
    } catch (err) { res.status(500).json({ message: 'Błąd pobierania danych.', error: err.message }); }
});

app.post('/api/reservations', async (req, res) => {
    try {
        const newReservation = {
            tableId: req.body.selected_seat_value, tableName: req.body.selected_seat_display.replace('Wybrano: ', ''),
...
app.get('/api/reservations', async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ message: 'Błąd pobierania rezerwacji.' }); }
});
...
app.delete('/api/reservations/:id', async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ message: "Błąd usuwania." }); }
});
...
app.post('/api/products', upload.single('imageFile'), async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ message: 'Błąd dodawania produktu.' }); }
});
...
app.put('/api/products/:id', upload.single('imageFile'), async (req, res) => {
    try {
        ...
        await createLog(req.body.currentUserLogin, 'Zaktualizowano produkt', `Nazwa: ${updatedData.name}`);
        const allProducts = await db.collection('products').find().toArray();
...
app.post('/api/work/stop', async (req, res) => {
    try {
        ...
        if (!session) { return res.status(404).json({ message: "Nie znaleziono aktywnej sesji." }); }
        const user = await db.collection('users').findOne({ id: userId });
        const endTime = new Date();
        const totalHours = (endTime - new Date(session.startTime)) / (1000 * 60 * 60);
